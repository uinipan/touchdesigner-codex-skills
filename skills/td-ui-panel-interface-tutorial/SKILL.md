...
When learning a series of UI tutorials, maintain one skill-level note instead of one note per video. Discover the user's actual note location instead of assuming a vault path. Append series evidence, timestamps, and reusable findings there, then link to concrete tutorial notes only when a single video becomes a standalone build note.
